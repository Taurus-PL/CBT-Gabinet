...
# --- NOWA BAZA ASYSTENTA DIAGNOZY ---
baza_symptomow = [
    {
        "diagnoza": "F50.2 Żarłoczność psychiczna (Bulimia)",
        "roznicowa": "Anoreksja (F50.0), BED, Depresja.",
        "akcje_czasowniki": ["jadł", "żar", "zjad", "wyjad", "wymiot", "rzyg", "przeczyszcz", "ćwicz", "bieg", "głod", "pochłan", "objad", "płak"],
        "profil_cbt": {
            "SYTUACJA (Wyzwalacz)": {
                "slowa": ["wieczór", "samotn", "stres", "kłótni", "imprez", "lustrz", "wag", "komentarz"],
                "tlumaczenie": "Ekspozycja na bodźce związane z sylwetką lub trudne sytuacje interpersonalne."
            },
            "MYŚLI (Sfera poznawcza - wyłapane myśli automatyczne)": {
                "slowa": ["grub", "śmieć", "nienawidz", "brzyd", "beznadziej", "nigdy", "zawsze", "muszę", "wszyscy", "nikt", "nie dam rady", "porażk"],
                "tlumaczenie": "Zniekształcenia poznawcze: myślenie dychotomiczne ('wszystko albo nic'), nadmierna generalizacja, surowa ocena własnej osoby."
            },
            "EMOCJE (Sfera emocjonalna)": {
                "slowa": ["wstyd", "wyrzut", "win", "lęk", "boję", "stres", "napięc", "złość", "smut"],
                "tlumaczenie": "Wstyd, poczucie winy, lęk przed utratą kontroli i przytyciem."
            },
            "CIAŁO (Sfera fizjologiczna)": {
                "slowa": ["zmęcz", "słab", "mdł", "zimn", "brzuch", "gardł", "opuch", "serc"],
                "tlumaczenie": "Wyczerpanie fizyczne, dolegliwości ze strony przewodu pokarmowego."
            }
        },
        "cele_smart": "1. Wprowadzenie regularnego planu posiłków.\n2. Zmniejszenie częstotliwości wymiotów do 1/tydz.",
        "protokol_nazwa": "CBT-E wg C. Fairburna",
        "uzasadnienie_planu": "1) Psychoedukacja (błędne koło). 2) Dzienniczek. 3) Restrukturyzacja poznawcza."
    },
    {
        "diagnoza": "F32 Epizod depresyjny",
        "roznicowa": "ChAD, Dystymia, Niedoczynność tarczycy.",
        "akcje_czasowniki": ["leż", "płacz", "śpi", "zaniedb", "unik", "wegetuj", "izoluj"],
        "profil_cbt": {
            "SYTUACJA (Wyzwalacz)": {
                "slowa": ["rano", "wsta", "prac", "obowiąz", "ludz", "problem"],
                "tlumaczenie": "Konieczność podjęcia aktywności, poranki (często gorsze samopoczucie rano)."
            },
            "MYŚLI (Sfera poznawcza - wyłapane myśli automatyczne)": {
                "slowa": ["beznadziej", "bez sensu", "nikim", "ciężar", "nie uda", "głup", "win", "nigdy", "zawsze", "wszystko", "nic"],
                "tlumaczenie": "Negatywna triada Becka, katastrofizacja, czarno-białe myślenie."
            },
            "EMOCJE (Sfera emocjonalna)": {
                "slowa": ["smut", "przygnęb", "płacz", "pust", "nic nie czuj"],
                "tlumaczenie": "Obniżony nastrój, anhedonia (brak odczuwania przyjemności)."
            },
            "CIAŁO (Sfera fizjologiczna)": {
                "slowa": ["spać", "zmęcz", "brak sił", "budzę się", "apetyt"],
                "tlumaczenie": "Spadek energii, zaburzenia snu, zmiana apetytu."
            }
        },
        ...
        "uzasadnienie_planu": "1) Monitorowanie i planowanie aktywności. 2) Restrukturyzacja poznawcza."
    },
    {
        ...
        "profil_cbt": {
            "SYTUACJA (Wyzwalacz)": {
                ...
                "tlumaczenie": "Miejsca zatłoczone, zamknięte przestrzenie, wysiłek fizyczny."
            },
            "MYŚLI (Sfera poznawcza - wyłapane myśli automatyczne)": {
                "slowa": ["umrę", "uduszę", "zawał", "zwariuję", "tracę kontrol", "zemdlej", "to koniec", "nigdy", "zawsze", "muszę"],
                "tlumaczenie": "Katastroficzna interpretacja normalnych doznań płynących z ciała."
            },
            "EMOCJE (Sfera emocjonalna)": {
            ...
            "CIAŁO (Sfera fizjologiczna)": {
                ...
                "tlumaczenie": "Silne pobudzenie wegetatywne (tachykardia, hiperwentylacja)."
            }
        },
        "cele_smart": "1. Zmniejszenie częstotliwości napadów do 0/miesiąc.\n2. Eliminacja zachowań zabezpieczających.",
        "protokol_nazwa": "Terapia Poznawcza Lęku Panicznego wg D. Clarka",
        "uzasadnienie_planu": "1) Reatrybucja doznań z ciała. 2) Eksperymenty behawioralne."
    }
]
...
if menu == "I. Diagnoza i Konceptualizacja":
    ...
    with st.expander("🤖 Asystent Diagnozy CBT", expanded=True):
        st.write("Wpisz swobodną wypowiedź pacjenta. Algorytm wyciągnie czasowniki, sfery i całe myśli w cudzysłowach.")
        ...
        if st.button("🔍 Analizuj i przygotuj konceptualizację CBT"):
            if objawy_input:
                ...
                # Dzielimy tekst na zdania do wyłapywania MYŚLI
                zdania_pacjenta = [zdanie.strip() for zdanie in re.split(r'[.!?]+', input_do_analizy) if zdanie.strip()]
                ...
                for choroba in baza_symptomow:
                    ...
                    if znalezione_akcje:
                        tymczasowa_lista += f"\n🏃‍♂️ WYKRYTE ZACHOWANIA (Akcje):\n- {', '.join(znalezione_akcje)}\n"
                        wynik_choroby += len(znalezione_akcje)
                    
                    # 2. Reszta sfer
                    for sfera, dane_sfery in choroba["profil_cbt"].items():
                        
                        # --- Wyciąganie całych zdań dla MYŚLI ---
                        if "MYŚLI" in sfera:
                            znalezione_mysli_zdania = []
                            for rdzen in dane_sfery["slowa"]:
                                for zdanie in zdania_pacjenta:
                                    if rdzen in zdanie and zdanie.capitalize() + "." not in znalezione_mysli_zdania:
                                        znalezione_mysli_zdania.append(zdanie.capitalize() + ".")
                            
                            if znalezione_mysli_zdania:
                                wynik_choroby += len(znalezione_mysli_zdania) * 2
                                tymczasowa_lista += f"\n🧠 {sfera}:\n"
                                for mysl in znalezione_mysli_zdania:
                                    tymczasowa_lista += f"  > \"{mysl}\"\n"
                                tymczasowa_lista += f"  [Zniekształcenia: {dane_sfery['tlumaczenie']}]\n"
                        
                        # --- Wyciąganie słów dla reszty sfer (Emocje, Ciało, Sytuacja) ---
                        else:
                            ...
                            if znalezione_pelne_slowa:
                                ...
                                tymczasowa_lista += f"\n👉 {sfera}:\n- Wykryto: '{', '.join(znalezione_pelne_slowa)}'\n- Opis CBT: {dane_sfery['tlumaczenie']}\n"
